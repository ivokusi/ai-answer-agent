...
export default function ChatPage() {
  ...
  const handleShare = async () => {
    try {
      ...
      await fetch('/api/conversations', {
        method: 'POST',
        body: JSON.stringify({ conversationId: newConversationId, messages }),
      });
      
      const newUrl = `${window.location.origin}/chat/${newConversationId}`;
      setShareUrl(newUrl);
    } catch (error) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await fetch('/api/conversations', {
        method: 'POST',
        body: JSON.stringify({ conversationId, messages: [...messages, { role: "user", content: message }, { role: "assistant", content: data.message }] }),
      });
          ...
          <div className="flex items-center gap-4">
            ...
            {shareUrl && (
              <div className="text-gray-300 text-sm">
                Shared URL: <a href={shareUrl} className="text-cyan-400 hover:underline" target="_blank" rel="noopener noreferrer">{shareUrl}</a>
              </div>
            )}
          </div>
        </div>
